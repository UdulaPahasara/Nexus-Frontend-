...
const ListItem = ({ icon, title, subtitle, date, description, darkMode, actionBtn }) => (
    <Box sx={{ display: 'flex', gap: '15px', mb: '20px', alignItems: 'flex-start', width: '100%' }}>
        <Box
            component="img"
            src={icon}
            sx={{ width: '45px', height: '45px', borderRadius: '5px', objectFit: 'cover', bgcolor: darkMode ? '#333' : '#fff' }}
        />
        <Box sx={{ display: 'flex', flexDirection: 'column', flexGrow: 1, gap: '2px' }}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <Typography sx={{ fontSize: '13px', fontWeight: 600, color: darkMode ? '#fff' : '#000', fontFamily: 'Poppins', lineHeight: 1.2 }}>
                    {title}
                </Typography>
                {actionBtn}
            </Box>
...
const UserAboout = ({ darkMode }) => {
    ...
    const SubSection = ({ title, action, children }) => (
        <Box sx={{
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: '12px' }}>
                ...
                {action && (
                    <Typography sx={{ fontSize: '12px', fontWeight: 600, color: '#00E87F', cursor: 'pointer', fontFamily: 'Poppins' }}>
                        {action}
                    </Typography>
                )}
            </Box>
    ...
    return (
        <Box sx={{ width: '100%', px: '23px', pb: '30px', boxSizing: 'border-box' }}>
            ...
            {/* Services */}
            <SubSection title="Services" action="View All">
            ...
            <SubSection title="Licenses & Certifications">
                <Box sx={{ mt: '15px' }}>
                    <ListItem
                        icon={icon5}
                        title="Microsoft"
                        ...
                        actionBtn={
                            <Button variant="outlined" sx={{
                                height: '26px', fontSize: '10px', borderRadius: '20px',
                                color: darkMode ? '#fff' : '#000',
                                borderColor: darkMode ? 'rgba(255,255,255,0.4)' : 'rgba(0,0,0,0.6)',
            ...
            {/* Projects */}
            <SubSection title="Projects" action="View All">
                <Box sx={{ mt: '15px' }}>
                    <ListItem
                        icon={icon1}
